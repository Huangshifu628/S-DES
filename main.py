...
def click():
    ...
    s = ''
    K1, K2 = generate_key(entry_content2, p10_table, p8_table)
    # 对明文进行加密

    #提取单个字符转换成ACSII码后连接成新的字符串
    for i in entry_content1:
        st = str.zfill(bin(ord(i))[2:],8)
        sr = encrypt(st, K1, K2)
        ss = chr(eval('0b'+str(sr)))
        s = s + ss

    #ciphertext1 = encrypt(entry_content1, K1, K2)
...
if __name__=="__main__":
 ...
 lab1=Label(root,text="  请输入明文：").grid(row=0)
 lab2=Label(root,text="  请输入密钥：").grid(row=1)
 lab3=Label(root,text="  可得到暗文：").grid(row=3)
 e1=Entry(root,width=25)#明文
 e2=Entry(root,width=25)#密钥
 e3=tk.Text(root,width=25,height=1)
 e1.grid(row=0,column=1)
 e2.grid(row=1,column=1)
 e3.grid(row=3,column=1)

 button1=tk.Button(root,text="确定",command=click)
 button1.grid(row=2)
